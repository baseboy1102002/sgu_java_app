...
public class Recieved_GUI extends javax.swing.JPanel implements checkPermission {
    ...
    private void initComponents() {
        ...
        if (tblProductList.getColumnModel().getColumnCount() > 0) {
            ...
            tblProductList.getColumnModel().getColumn(1).setPreferredWidth(10);
            tblProductList.getColumnModel().getColumn(4).setPreferredWidth(15);
        }
        ...
        Warning.setText("Bạn nhập quá số lượng nhà cung cấp có rồi!!");
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                while (rowIterator.hasNext()) {
                        ...
                        if (row.getRowNum() != 0) {
                            switch (cell.getColumnIndex()) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    price = (double) cell.getNumericCellValue();
                                    break;
                                case 4:
                    ...
                    if (!"".equals(id)) {
                        ReceivedProduct_DTO rProduct = new ReceivedProduct_DTO(id, name, size, price,quantity);
...
//            checkDetails();
    ...
    private void btnAdd1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnAdd1MouseClicked
        ...
        if (!listReceivedProductDetail.contains(selectedProduct)) {
            if(productBUS.checkProduct(selectedProduct.getProductID(),selectedProduct.getSize()))
                {
...
//                checkDetails();
                ...
                JOptionPane.showMessageDialog(this, "Sản phẩm không tồn tại hoặc đã bị xóa!");
            }
        } else {
        ...
        for(int i=0;i<tblReceiveDetail.getRowCount();i++){        
            try{
                if (tblReceiveDetail.getValueAt(i, 4) == null || tblReceiveDetail.getValueAt(i, 4).toString().trim().length() == 0 || Integer.parseInt(tblReceiveDetail.getValueAt(i, 4).toString()) <= 0) {
                    ...
                    int inputQuantity=(int)tblReceiveDetail.getValueAt(i, 4);
                    String id =(String) tblReceiveDetail.getValueAt(i,0);
                    String size = (String)tblReceiveDetail.getValueAt(i,1);
                    for(ReceivedProduct_DTO rp:listReceivedProduct){
                        if (rp.getProductID().equalsIgnoreCase(id) && rp.getSize().equalsIgnoreCase(size)){
                            if(rp.getQuantity()<inputQuantity){
                ...
                if(flag){
                        double price=(double) tblReceiveDetail.getValueAt(i,3);
                        total+= price*(int)tblReceiveDetail.getValueAt(i, 4);
                        tax=total*0.08;
    ...
    private void btnNhapHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapHangActionPerformed
        ...
        if(insert==true){
            ...
            for (ReceivedProduct_DTO rp : listReceivedProductDetail) {
                double totalPrice = rp.getPrice() * quantityList.get(counter);
                ...
                if (!receiveDetailBUS.insert(rnd)) {
                    JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi trong quá trình thêm!");
                    receiveBUS.delete(receivednoteID);
                    insert=false;
                    break;
                }
                counter++;
        ...
        if(insert==true){
            int counter = 0;
            for (ReceivedProduct_DTO rp : listReceivedProductDetail) {
                if (!productBUS.incQuantity(rp.getProductID(), rp.getSize(), quantityList.get(counter))) {
                    JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi trong quá trình thêm!");
                    receiveBUS.delete(receivednoteID);
                    insert=false;
                    break;
    ...
    private void loadReceivedProducts(ArrayList<ReceivedProduct_DTO> list) {
        ...
        model.setRowCount(0);
        for (ReceivedProduct_DTO rProduct : list) {
            model.addRow(new Object[]{rProduct.getProductID(), rProduct.getSize(), rProduct.getProductName(), rProduct.getPrice(), rProduct.getQuantity()});
        }
    }
