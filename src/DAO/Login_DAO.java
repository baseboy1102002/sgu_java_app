...
public class Login_DAO{
    ...
    public Account login(String username, String password) {
        String sql = "SELECT * FROM `account` WHERE `UserName`=? AND `Password`=? AND 'IsDeleted'!=1";
        Account acc = null;
    ...
    public ArrayList<String> getStaffInfo(String username, String password) {
        ...
        + " LEFT JOIN decentralization ON account.decentralize_id = decentralization.decentralize_id WHERE UserName =? AND Password =? AND staff.IsDeleted!=1";
        try(Connection conn = cn.getConnect(); PreparedStatement pstm = conn.prepareStatement(sql);) {
            pstm.setString(1, username);
            pstm.setString(2, password);
            ResultSet rs = pstm.executeQuery();
